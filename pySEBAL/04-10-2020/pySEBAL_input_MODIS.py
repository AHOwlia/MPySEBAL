...
def Get_MODIS_Para_Veg(workbook, number, Example_fileName, year, month, day, path_radiance, Apparent_atmosf_transm, cos_zn, dr, DEM_resh, epsg_to):
    ...
    try:
        if (ws['B%d' % number].value) is not None:
			...
			#--------owlia ----- add selection and crop to ROI for NDVI --- not work yet
            #ws = workbook['General_Input']
            #MOD_3 =int(ws['j%d' %number].value)
            #if (MOD_3==1) :
            #    Output_filename_dataset3 = os.path.join(output_folder, 'Owlia-ROI', 'Owlia-ROI.tif')
            #    dataset3, dataset3_source = SEBAL.Open_constant_or_spatial_map(ws, "k%d" %number, Output_filename_dataset3, Example_fileName)
            #    NDVI[dataset3 == 0] = np.nan
            #ws = workbook['Additional_Input'] #--owlia --- add to back it to code
			#-------owlia--------
            # NDVI stats
    ...
    try:
        ...
        # if the users surface albedo data cannot be reprojected than use the original MODIS data as imported into SEBAL
			...
			#--------owlia ----- add selection and crop to ROI    for albedo  --- not work yet
            #ws = workbook['General_Input']
            #MOD_3 =int(ws['j%d' %number].value)
            #if (MOD_3==1) :
            #    Output_filename_dataset3 = os.path.join(output_folder, 'Owlia-ROI', 'Owlia-ROI.tif')
            #    dataset3, dataset3_source = SEBAL.Open_constant_or_spatial_map(ws, "k%d" %number, Output_filename_dataset3, Example_fileName)
            #    Surf_albedo[dataset3 == 0] = np.nan
            #ws = workbook['Additional_Input'] #--owlia --- add to back it to code
			#-------owlia--------
...
def Get_MODIS_Para_Thermal(workbook, number, Example_fileName, year, month, day, water_mask_temp, b10_emissivity, Temp_inst,  Rp, tau_sky, surf_temp_offset, Thermal_Sharpening_not_needed, epsg_to):
    ...
    try:
        ...
        if ws['D%d' % number].value is None:
			...
			#--------owlia ----- add selection and crop to ROI     for LST    --- not work yet
            #ws = workbook['General_Input']
            #MOD_3 =int(ws['j%d' %number].value)
            #if (MOD_3==1) :
            #    Output_filename_dataset3 = os.path.join(output_folder, 'Owlia-ROI', 'Owlia-ROI.tif')
            #    dataset3, dataset3_source = SEBAL.Open_constant_or_spatial_map(ws, "k%d" %number, Output_filename_dataset3, Example_fileName)
            #    Surface_temp[dataset3 == 0] = np.nan
            #ws = workbook['Additional_Input'] #--owlia --- add to back it to code
			#-------owlia--------
