...
import warnings
import scipy #owlia added for percentile at selection of anchor pixels
...
def main(number, inputExcel):
    ...
    Image_Type = int(ws['D%d' %number].value)           # Type of Image (1=Landsat & 2 = VIIRS & PROBA-V & 3 = MODIS)
    ...
    # Start log file
    filename_logfile = os.path.join(output_folder, '%s_log.txt' %(number-1))     #owlia: name changed to have number of run in log file name
	...
	#-----owlia----------------------         added dynamic version of treshold for it, its only work when hot pixel selection mode=1, in spreadshit 'T' cell
    ws = wb['General_Input']
    NDVIhot_low =float(ws['l%d' %number].value)
    NDVIhot_high =float(ws['m%d' %number].value)
	#------------------------------------
	...
	#-----owlia----------------------         added dynamic version of model use for it, raupach=2 , ndvi=1, in urmia 2 was better
    surf_roughness_equation_used =int(ws['n%d' %number].value)
	#------------------------------------
    ...
    DEM_resh[DEM_resh == -9999] = np.nan            # owlia added for no values in DEM files
	...
	#----------------------------owlia---------------       check  user or default items for run - Want to force select from cold pixels we set?    0=no  1=yes         
    ws = wb['General_Input']
    MOD_1 =int(ws['f%d' %number].value)
    if MOD_1==1:
	
    #----------------------------owlia---------------       Part of code that force to use our dataset of coldpixel- this part get address of tif and dataset
	    ws = wb['General_Input']	
	    Output_filename_dataset1 = os.path.join(output_folder, 'Owlia-Developed_1', 'Coldpixel-ForcedArea.tif')
	    dataset1, dataset1_source = Open_constant_or_spatial_map(ws, "g%d" %number, Output_filename_dataset1, proyDEM_fileName)        
    ...
    ws = wb['General_Input']
    MOD_3 =int(ws['r%d' %number].value)
    if MOD_3==1:
	
    #----------------------------owlia---------------       Part of code that force to use our dataset of Hotpixel
	    ws = wb['General_Input']	
	    Output_filename_dataset3 = os.path.join(output_folder, 'Owlia-Developed_1', 'HotPixels-ForcedArea.tif')
	    dataset3, dataset3_source = Open_constant_or_spatial_map(ws, "s%d" %number, Output_filename_dataset3, proyDEM_fileName)        
...
#----------owlia --------------------   cold pixel improved, if you use auto select cold pixel, do you want to pick it auto or manual?  Auto default=0, Auto Improved=1, water only=2, veg only=3			
        ws = wb['General_Input']

        
        MOD_4 = int(ws['o%d' %number].value)
		
        if MOD_4==0:    # default
            ts_dem_cold_veg_mean, cold_pixels_vegetation = Calc_Cold_Pixels_Veg_default(NDVI, NDVI_max, NDVI_std, QC_Map, ts_dem, dataset1)
            ts_dem_cold_mean, cold_pixels = Calc_Cold_Pixels_default(ts_dem, water_mask, QC_Map, ts_dem_cold_veg_mean, cold_pixels_vegetation,dataset1)
		
        if MOD_4==1:    #improved
            ANDVI = int(ws['p%d' %number].value)  #allen et al. 2013 NDVI
            ALST = int(ws['q%d' %number].value)  #allen et al. 2013 LST
            ts_dem_cold_veg_mean, cold_pixels_vegetation = Calc_Cold_Pixels_Veg_improved(NDVI, NDVI_max, NDVI_std, QC_Map, ts_dem, dataset1, water_mask, ANDVI, ALST)
            ts_dem_cold_mean, cold_pixels = Calc_Cold_Pixels_improved(ts_dem, water_mask, QC_Map, ts_dem_cold_veg_mean, cold_pixels_vegetation,dataset1)
		
        if MOD_4==2:     #water only
            ts_dem_cold_mean, cold_pixels = Calc_Cold_Pixels_onlywater(ts_dem, water_mask, QC_Map,dataset1)		    
		
        if MOD_4==3:      #vegetation only
            ANDVI = int(ws['p%d' %number].value)  #allen et al. 2013 NDVI
            ALST = int(ws['q%d' %number].value)  #allen et al. 2013 LST
            ts_dem_cold_mean, cold_pixels = Calc_Cold_Pixels_VegOnly(NDVI, NDVI_max, NDVI_std, QC_Map, ts_dem, dataset1, water_mask, ANDVI, ALST)
        ...
        ws = wb['Additional_Input']   	
		                                                    ...
		                                                    #------origin have -------------------owlia ---> add dataset 1 to water and veg function---- + watermask!	
        # Cold pixels vegetation
        #ts_dem_cold_veg_mean, cold_pixels_vegetation = Calc_Cold_Pixels_Veg(NDVI, NDVI_max, NDVI_std, QC_Map, ts_dem, dataset1, water_mask)  #origin
        #save_GeoTiff_proy(lsc, cold_pixels_vegetation, cold_pixels_fileName, shape_lsc, nband=1) #owlia -  check veg

        # Cold pixels water
        #ts_dem_cold_mean, cold_pixels = Calc_Cold_Pixels(ts_dem, water_mask, QC_Map, ts_dem_cold_veg_mean, cold_pixels_vegetation,dataset1)
        #------------------------------------------------------------																				origin
        ...
        save_GeoTiff_proy(lsc, cold_pixels, cold_pixels_fileName, shape_lsc, nband=1)      #origin
    ...
    if (ws['H%d' % number].value) is not None:
        ...
        # Hot pixels
        ws = wb['General_Input']
        MOD_5 = int(ws['t%d' %number].value)  #owlia: mode selection default 1, improved 2 (allen et al. 2013 automated=2)
        
        
        if MOD_5 == 1:
            ts_dem_hot_mean, hot_pixels = Calc_Hot_Pixels(ts_dem, QC_Map, water_mask, NDVI, NDVIhot_low, NDVIhot_high, ts_dem_cold_mean, dataset3)
            
        if MOD_5 == 2:
            ANDVI2 = int(ws['u%d' %number].value)  #allen et al. 2013 NDVI
            ALST2 = int(ws['v%d' %number].value)  #allen et al. 2013 LST
            ts_dem_hot_mean, hot_pixels = Calc_Hot_Pixels_improved(ts_dem, QC_Map, water_mask, NDVI, ANDVI2, ALST2, ts_dem_cold_mean, dataset3)
    ...
    # Correct ETA_24 based on slope (basal crop coefficient)    #owlia change from comment to run  - FAO56
	...
	#----------------------------owlia---------------         opening basin raster and extract by mask it via our dataset after calculation for ET
    ws = wb['General_Input']
    MOD_2 =int(ws['h%d' %number].value)
    if MOD_2==1:
        Output_filename_dataset2 = os.path.join(output_folder, 'Owlia-Developed_1', 'Extract_ETaArea.tif')
        dataset2, dataset2_source = Open_constant_or_spatial_map(ws, "i%d" %number, Output_filename_dataset2, proyDEM_fileName)
        ETA_24[dataset2 == 0.0] = np.nan
        ETA_24[dataset2 == np.nan] = np.nan
	
	#------------------------------------------------         produce monthly ETa based on Cumulative ETref excel value
    ws = wb['General_Input']
    MonthlyReport1 =int(ws['x%d' %number].value)
    if MonthlyReport1==1:
        Output_filename_monthly = os.path.join(output_folder, 'Owlia-Developed_2', '1-MonthlyETa_%s%02d.tif' %(year, month))
        CumulativeETrf, CumulativeETrf_source = Open_constant_or_spatial_map(ws, "y%d" %number, Output_filename_monthly, proyDEM_fileName)
		
        monthlyETa = kc * CumulativeETrf
        if MOD_2==1:
            monthlyETa[dataset2 == 0.0] = np.nan
            monthlyETa[dataset2 == np.nan] = np.nan
        
        save_GeoTiff_proy(lsc, monthlyETa, Output_filename_monthly, shape_lsc, nband=1)
		
        print('Cumulative ETrf defined by user for multiply : %0.3f mm/month' %CumulativeETrf)
		
        ETaMO_min = np.nanmin(monthlyETa)    # Standard deviation
        ETaMO_max = np.nanmax(monthlyETa)    # Max
        ETaMO_mean = np.nanmean(monthlyETa)  # Mean
        ETaMO_std = np.nanstd(monthlyETa)    # Standard deviation
	
        print('ETa Monthly min: %0.3f mm/month' %ETaMO_min)
        print('ETa Monthly max: %0.3f mm/month' %ETaMO_max)
        print('ETa Monthly mean: %0.3f mm/month' %ETaMO_mean)
        print('ETa Monthly std: %0.3f mm/month' %ETaMO_std)
		
	
	
    ETa24_min = np.nanmin(ETA_24)    # Standard deviation
    ETa24_max = np.nanmax(ETA_24)    # Max
    ETa24_mean = np.nanmean(ETA_24)  # Mean
    ETa24_std = np.nanstd(ETA_24)    # Standard deviation
	
    print('ETa 24 min: %0.3f mm/day' %ETa24_min)
    print('ETa 24 max: %0.3f mm/day' %ETa24_max)
    print('ETa 24 mean: %0.3f mm/day' %ETa24_mean)
    print('ETa 24 std: %0.3f mm/day' %ETa24_std)
	#----------------------------------------------seprate landcover for ET   owlia
    ws = wb['General_Input']
    LCseprate =int(ws['z%d' %number].value)
    if LCseprate==1:
        print('-----using sepration with landcover maps: -----')
        Output_filename_IrrAgri1LC = os.path.join(output_folder, 'Owlia-Developed_2', '2-IrrAgri1LC.tif')
        IrrAgri1, IrrAgri1_source = Open_constant_or_spatial_map(ws, "aa%d" %number, Output_filename_IrrAgri1LC, proyDEM_fileName)
        ET24copy = np.copy(ETA_24)
        ET24copy[IrrAgri1 == 0.0] = np.nan
        ET24copy[IrrAgri1 == np.nan] = np.nan
        ETa24lc_sum = np.nansum(ET24copy)
        print('ETa24 for irrigated agricultre sum: %0.3f mm/day' %ETa24lc_sum)
        save_GeoTiff_proy(lsc, ET24copy, Output_filename_IrrAgri1LC, shape_lsc, nband=1)
...
def Corr_ETact(ETA_24, ETpot_24, NDVI, slope):
    ...
    ETA_24=ETA_24.clip(0,15.0)  #owlia added for dont save NaN
...
#------------------------------------------------------------------------------
def Calc_Hot_Pixels(ts_dem, QC_Map, water_mask, NDVI, NDVIhot_low, NDVIhot_high, ts_dem_cold, dataset3):
    """
    ...
    hot_pixels[dataset3 == 0.0] = np.nan            #---owlia force hot pixels to cut
    hot_pixels[dataset3 == np.nan] = np.nan             #---owlia force hot pixels to cut
...
#------------------------------------------------------------------------------
# def Calc_Cold_Pixels(ts_dem, water_mask, QC_Map, ts_dem_cold_veg_mean, cold_pixels_vegetation,dataset1):
    # """
    ...
    # cold_pixels[dataset1 == 0.0] = np.nan            #------owlia-------	    force code to use cold pixels that include in our dataset
    # cold_pixels[dataset1 == np.nan] = np.nan         #------owlia-------	    force code to use cold pixels that include in our dataset
    ...
    # cold_pixels[cold_pixels > ts_dem_cold_mean] = np.nan
...
#------------------------------------------------------------------------------
# def Calc_Cold_Pixels_Veg(NDVI, NDVI_max, NDVI_std, QC_Map, ts_dem, dataset1, water_mask):
    # """
    # Function to calculates the the cold pixels based on vegetation
    # """
    # cold_pixels_vegetation = np.copy(ts_dem)
	# #---owlia------ new adeded
    # #cold_pixels_vegetation[NDVI <= 0.5] = 0.0
    # cold_pixels_vegetation[NDVI == np.nan] = 0.0
    # cold_pixels_vegetation[NDVI == 0.0] = 0.0
    # cold_pixels_vegetation[QC_Map != 0.0] = 0.0
    # cold_pixels_vegetation[water_mask == 1.0] = 0.0
	
	# #-------owlia new added

    # #cold_pixels_vegetation[np.logical_or(NDVI <= (NDVI_max-0.1*NDVI_std),QC_Map != 0.0)] = 0.0 #origin

    # #cold_pixels_vegetation[np.logical_or(NDVI <= (0.8*NDVI_max),QC_Map != 0.0)] = 0.0     	#---owlia change std to 0.9 max-------   Allen et al.2013  khodam ezafe kardam
	
	# # owlia ---------- (the coldest 20% of the 5% highest vegetated pixels)
    # NDVI = NDVI*1000
    # high_ndvi = scipy.stats.scoreatpercentile(NDVI, 95)
    # NDVI= NDVI/1000
    # high_ndvi= high_ndvi/1000
    # print('Owlia develop top 5ndvi first:%0.3f ' %high_ndvi)
    # cold_pixels_vegetation[NDVI < high_ndvi] = 0.0
    ...
    # cold_pixels_vegetation[dataset1 == 0.0] = np.nan        #------owlia-------	    force code to use cold pixels that include in our dataset
    # cold_pixels_vegetation[dataset1 == np.nan] = np.nan     #------owlia-------	    force code to use cold pixels that include in our dataset
	
	# #---------owlia -------  20% coldest pixels
    # cold_pixels_vegetation = cold_pixels_vegetation * 100000
    # cold_ndvi = scipy.stats.scoreatpercentile(cold_pixels_vegetation, 20)
    # cold_pixels_vegetation[cold_pixels_vegetation > cold_ndvi] = 0.0               #this is correct but not work for this modis!
    # cold_pixels_vegetation[cold_pixels_vegetation==0.0] = np.nan
    # cold_pixels_vegetation = cold_pixels_vegetation / 100000
    # cold_ndvi = cold_ndvi / 100000
    # print('Owlia develop top 20coldest:%0.3f ' %cold_ndvi)
	
    # #ts_dem_cold_std_veg = np.nanstd(cold_pixels_vegetation)
    # #ts_dem_cold_min_veg = np.nanmin(cold_pixels_vegetation)                                             #this is not correct but may need later!
    # #cold_pixels_vegetation[cold_pixels_vegetation > (ts_dem_cold_min_veg+(2*ts_dem_cold_std_veg))] = 0.0
    # #cold_pixels_vegetation[cold_pixels_vegetation==0.0] = np.nan
	# #------------
	...
	# #----owlia remove high cold pixels-----
   # # ts_dem_cold_std_veg = np.nanstd(cold_pixels_vegetation)
   # # ts_dem_cold_min_veg = np.nanmin(cold_pixels_vegetation)
   # # ts_dem_cold_mean_veg = np.nanmean(cold_pixels_vegetation)
    # #ts_dem_cold_mean_veg = 302.00
	# #--------------------------------------
	
    # #cold_pixels_vegetation[cold_pixels_vegetation > ts_dem_cold_mean_veg] = np.nan        # origin have it owlia
...
#------------------------------------------------------------------------------
def Calc_albedo(Reflect,path_radiance,Apparent_atmosf_transm, LS_number):
    """
...
#------------------------------------------------------------------------------
	...
	#                               Owlia added functions
	
	# default code function   (dataset 1 added for force)
	
def Calc_Cold_Pixels_default(ts_dem, water_mask, QC_Map, ts_dem_cold_veg_mean, cold_pixels_vegetation,dataset1):
    """
    Function to calculates the the cold pixels based on the surface temperature
    """
    for_cold = np.copy(ts_dem)
    ...
    #----- owlia added final count of cold pixels after force
    countcolds3 =  np.count_nonzero(cold_pixels > 0)
    print('developed- count of cold pixels found at last (prefer water in this type): %0.0f ' %countcolds3)
    #------------
...
#------------------------------------------------------------------------------
def Calc_Cold_Pixels_Veg_default(NDVI, NDVI_max, NDVI_std, QC_Map, ts_dem, dataset1):
    """
    Function to calculates the the cold pixels based on vegetation
    """
    print('use force code to select on default for cold pixels (0 model in excel)')
    cold_pixels_vegetation = np.copy(ts_dem)
    ...
    #----- owlia added count of cold pixels before force
    countcolds1 =  np.count_nonzero(cold_pixels_vegetation > 0)
    print('developed- count of cold pixels of veg before force and potential can be: %0.0f ' %countcolds1)
    #------------	
    cold_pixels_vegetation[dataset1 == 0.0] = np.nan        #------owlia-------	    force code to use cold pixels that include in our dataset
    cold_pixels_vegetation[dataset1 == np.nan] = np.nan     #------owlia-------	    force code to use cold pixels that include in our dataset
    
    #----- owlia added final count of cold pixels after force
    countcolds2 =  np.count_nonzero(cold_pixels_vegetation > 0)
    print('Owlia count of cold pixels of veg found after force on select: %0.0f ' %countcolds2)
    #------------
...
#-------------------------------------------------------------------------------------
    ...
    #improved from default auto
	
def Calc_Cold_Pixels_improved(ts_dem, water_mask, QC_Map, ts_dem_cold_veg_mean, cold_pixels_vegetation,dataset1):
    """
    Function to calculates the the cold pixels based on the surface temperature
    """
    for_cold = np.copy(ts_dem)
    ...
    cold_pixels[for_cold < 278.0] = np.nan
    cold_pixels[for_cold > 320.0] = np.nan
    # cold_pixels[for_cold < 285.0] = 285.0  #  Karimi 2019 used it
...
#------------------------------------------------------------------------------
def Calc_Cold_Pixels_Veg_improved(NDVI, NDVI_max, NDVI_std, QC_Map, ts_dem, dataset1, water_mask, ANDVI, ALST):
    """
    Function to calculates the the cold pixels based on vegetation
    """
    print('use force code to select improved auto for cold pixels (1 model in excel)')
    cold_pixels_vegetation = np.copy(ts_dem)
	#---owlia------ new adeded
    #cold_pixels_vegetation[NDVI <= 0.5] = 0.0
    cold_pixels_vegetation[NDVI == np.nan] = 0.0
	...
	# owlia ---------- (the coldest 20% of the 5% highest vegetated pixels) allen et al. 2013
    
    #ndvi2 = np.copy(NDVI)
    #ndvi2[np.isnan(ndvi2)] = 0.0
    #high_ndvi = scipy.stats.scoreatpercentile(ndvi2, ANDVI)
    high_ndvi = scipy.stats.scoreatpercentile(NDVI[~np.isnan(NDVI)], ANDVI)
    print('developed- top %0.0f percent' %ANDVI , 'NDVI:%0.3f' %high_ndvi)
    cold_pixels_vegetation[NDVI < high_ndvi] = 0.0
    cold_pixels_vegetation[cold_pixels_vegetation==0.0] = np.nan
	#-----owlia added up
	...
	#---------owlia -------  20% coldest pixels LST allen et al. 2013
    #cold_pixels_vegetation = cold_pixels_vegetation * 100000
    
    cold_ndvi = scipy.stats.scoreatpercentile(cold_pixels_vegetation[~np.isnan(cold_pixels_vegetation)], ALST)
    cold_pixels_vegetation[cold_pixels_vegetation > cold_ndvi] = 0.0               #this is correct but not work for this modis!
    cold_pixels_vegetation[cold_pixels_vegetation==0.0] = np.nan
    ...
    print('developed- lowest %0.0f percent' %ALST , 'LST:%0.3f K' %cold_ndvi)
    ...
    #----- owlia added count of cold pixels before force
    countcolds1 =  np.count_nonzero(cold_pixels_vegetation > 0)
    print('developed- count of cold pixels veg before force and potential can be: %0.0f ' %countcolds1)
    #------------	
    ...
    #----- owlia added final count of cold pixels after force
    countcolds2 =  np.count_nonzero(cold_pixels_vegetation > 0)
    print('developed- count of cold pixels of veg found after force on select: %0.0f ' %countcolds2)
    #------------
...
def Calc_Cold_Pixels_onlywater(ts_dem, water_mask, QC_Map,dataset1):
    """
    Function to calculates the the cold pixels based on the surface temperature
    """
    print('use force code to select only water for cold pixels (2 model in excel)')
    for_cold = np.copy(ts_dem)
    for_cold[water_mask != 1.0] = 0.0
    for_cold[QC_Map != 0] = 0.0
    cold_pixels = np.copy(for_cold)
    ...
    #----- owlia added count of cold pixels before force
    countcolds1 =  np.count_nonzero(cold_pixels > 0)
    print('developed- count of cold pixels before force and potential can be: %0.0f ' %countcolds1)
    #------------
    ...
    #----- owlia added final count of cold pixels after force
    countcolds2 =  np.count_nonzero(cold_pixels > 0)
    print('developed- count of cold pixels found at last: %0.0f ' %countcolds2)
    #------------
...
def Calc_Cold_Pixels_VegOnly(NDVI, NDVI_max, NDVI_std, QC_Map, ts_dem, dataset1, water_mask, ANDVI, ALST):
    """
    Function to calculates the the cold pixels based on vegetation
    """
    cold_pixels_vegetation = np.copy(ts_dem)
	#---owlia------ new adeded
    print('use force code to select only Vegetation for cold pixels (3 model in excel)')
    cold_pixels_vegetation[NDVI == np.nan] = 0.0
    cold_pixels_vegetation[NDVI == 0.0] = 0.0
    cold_pixels_vegetation[QC_Map != 0.0] = 0.0
    cold_pixels_vegetation[water_mask == 1.0] = 0.0
    ...
    cold_ndvi = scipy.stats.scoreatpercentile(cold_pixels_vegetation[~np.isnan(cold_pixels_vegetation)], ALST)
    cold_pixels_vegetation[cold_pixels_vegetation > cold_ndvi] = 0.0               
    cold_pixels_vegetation[cold_pixels_vegetation==0.0] = np.nan
    ...
    ts_dem_cold_mean = ts_dem_cold_mean_veg
    cold_pixels = cold_pixels_vegetation
...
def Calc_Hot_Pixels_improved(ts_dem, QC_Map, water_mask, NDVI, ANDVI2, ALST2, ts_dem_cold, dataset3):
    """
    Function to calculates the hot pixels based on the surface temperature and NDVI
    """
    print('use allen automate select for hot pixel ( model 2  in excel)')
    for_hot = np.copy(ts_dem)
    for_hot[np.logical_or(water_mask != 0.0, QC_Map != 0.0)] = 0.0
    hot_pixels = np.copy(for_hot)
    hot_pixels[for_hot < ts_dem_cold] = np.nan
    ...
    #------owlia----------- hotpixel allen et al. 2013 check NDVI
    low_ndvi = scipy.stats.scoreatpercentile(NDVI[~np.isnan(NDVI)], ANDVI2)
    print('developed- low %0.0f percent' %ANDVI2 , 'NDVI:%0.3f' %low_ndvi)
    hot_pixels[NDVI > low_ndvi] = 0.0
    hot_pixels[hot_pixels==0.0] = np.nan
    #---------------------- hotpixels allen check lst
    hot_ndvi = scipy.stats.scoreatpercentile(hot_pixels[~np.isnan(hot_pixels)], ALST2)
    hot_pixels[hot_pixels < hot_ndvi] = 0.0               
    hot_pixels[hot_pixels==0.0] = np.nan
    print('developed- highest %0.0f percent' %ALST2 , 'LST:%0.3f K' %hot_ndvi)
    ...
    #----- owlia added count of hot pixels before force   allen et al. 2013
    counthot1 =  np.count_nonzero(hot_pixels > 0)
    print('developed- count of hot pixels before force and potential can be: %0.0f ' %counthot1)
    #------------
    
    hot_pixels[dataset3 == 0.0] = np.nan            #---owlia
    hot_pixels[dataset3 == np.nan] = np.nan             #---owlia
    
    #----- owlia added final count of cold pixels after force
    counthot2 =  np.count_nonzero(hot_pixels > 0)
    print('developed- count of hot pixels found at last: %0.0f ' %counthot2)
    #------------
